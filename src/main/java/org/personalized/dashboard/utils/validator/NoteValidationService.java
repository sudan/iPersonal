...
public class NoteValidationService implements ValidationService<Note> {
    ...
    private void validateTitleLength(Note note, List<ErrorEntity> errorEntities) {
        if(note.getTitle().length() > Constants.NOTE_TITLE_MAX_LENGTH){
            ErrorEntity errorEntity = new ErrorEntity(ErrorCodes.MAX_TITLE_LENGTH_EXCEEDED.name(),
                    MessageFormat.format(ErrorCodes.MAX_TITLE_LENGTH_EXCEEDED.getDescription(), Constants.NOTE_TITLE_MAX_LENGTH));
    ...
    private void validateContentLength(Note note, List<ErrorEntity> errorEntities) {
        if(note.getNote().length() > Constants.NOTE_CONTENT_MAX_LENGTH) {
            ErrorEntity errorEntity = new ErrorEntity(ErrorCodes.MAX_CONTENT_LENGTH_EXCEEDED.name(),
                    MessageFormat.format(ErrorCodes.MAX_CONTENT_LENGTH_EXCEEDED.getDescription(), Constants.NOTE_CONTENT_MAX_LENGTH));
            errorEntities.add(errorEntity);
        }
    }
}
